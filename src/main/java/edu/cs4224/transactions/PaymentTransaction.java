...
public class PaymentTransaction extends BaseTransaction {
  ...
  @Override public void execute(final String[] dataLines) {
    ...
    District district = districtCollection.find(
        and(eq("d_W_ID", customer_warehouse_id), eq("d_ID", customer_district_id))).first();
    ...
    Customer customer = customerCollection.find(
        and(eq("c_W_ID", customer_warehouse_id), eq("c_D_ID", customer_district_id), eq("c_ID", customer_id))).first();
    ...
    System.out.println(String.format(
        "1. (C_W_ID: %d, C_D_ID: %d, C_ID: %d), Name: (%s, %s, %s), Address: (%s, %s, %s, %s, %s), C_PHONE: %s, C_SINCE: %s, C_CREDIT: %s, C_CREDIT_LIM: %.2f, C_DISCOUNT: %.4f, C_BALANCE: %.2f",
        customer_warehouse_id, customer_district_id, customer_id, customer.getC_FIRST(), customer.getC_MIDDLE(),
        customer.getC_LAST(),
    ...
    System.out.println(String.format("2. Warehouse: %s, %s, %s, %s, %s",
        warehouse.getW_STREET_1(), warehouse.getW_STREET_2(), warehouse.getW_CITY(), warehouse.getW_STATE(),
        warehouse.getW_ZIP()));

    System.out.println(String.format("3. District: %s, %s, %s, %s, %s",
        district.getD_STREET_1(), district.getD_STREET_2(), district.getD_CITY(), district.getD_STATE(),
        district.getD_ZIP()));
