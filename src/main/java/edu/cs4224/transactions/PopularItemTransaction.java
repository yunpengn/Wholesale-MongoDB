...
public class PopularItemTransaction extends BaseTransaction {
  ...
  public void execute(final String[] dataLines) {
    ...
    FindIterable<CustomerOrder> S = CustomerOrder.getCollection(db)
        .find(and(eq("o_D_ID", D_ID), eq("o_W_ID", W_ID), in("o_ID", O_ID_list)));
    ...
    S.forEach((Consumer<CustomerOrder>) order -> {
      ...
      Customer customer = Customer.getCollection(db)
          .find(and(eq("c_W_ID", W_ID), eq("c_D_ID", D_ID), eq("c_ID", order.getO_C_ID())))
          .first();
