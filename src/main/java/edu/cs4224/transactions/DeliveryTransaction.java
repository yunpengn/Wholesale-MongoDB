    ...
    for (int i = 1; i <= NUM_DISTRICTS; i++) {
      ...
      if (currentDistrict == null) {
        throw new RuntimeException(
            String.format("Unable to find district with warehouseID=%d districtID=%d", warehouseID, i));
      }
      ...
      if (yetDeliverOrder == null) {
        throw new RuntimeException(
            String.format("Unable to find order with warehouseID=%d districtID=%d orderID=%d", warehouseID, i,
                orderID));
      }
      ...
      order.updateOne(Filters.and(
          ...
          Filters.eq("o_D_ID", i),
          Filters.eq("o_ID", orderID)
      ), Updates.combine(Updates.set("o_CARRIER_ID", carrierID),
          Updates.set("o_L_INFO", yetDeliverOrder.getO_L_INFO())));
