  ...
  public void execute(final String[] dataLines) {
    ...
    customers.forEach((Consumer<Customer>) customer -> {
      ...
      District district = districtCollection.find(
          and(eq("d_W_ID", customer.getC_W_ID()), eq("d_ID", customer.getC_D_ID()))).first();

      System.out.println(
          String.format("Customer: Name(%s, %s, %s), Balance(%f), Warehouse(%s), District(%s)", customer.getC_LAST(),
              customer.getC_MIDDLE(), customer.getC_LAST(), customer.getC_BALANCE(), warehouse.getW_NAME(),
              district.getD_NAME()));
    });
  }
}
