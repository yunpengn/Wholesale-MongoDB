  ...
  private void customer_order() throws Exception {
    ...
    readAndExecute("order", row -> {
      ...
      for (OrderLineInfo info : infoList.values()) {
        ...
        set.add(String.format("'%d-%d-%d-%d'", customerOrder.getO_W_ID(), customerOrder.getO_D_ID(),
            customerOrder.getO_ID(), customerOrder.getO_C_ID()));
      ...
      for (int districtID : entry.getValue()) {
        ...
        if (yetToDeliver == null) {
          throw new RuntimeException(
              String.format("Cannot find yet-to-deliver order in warehouseID=%d districtID=%d", warehouseID,
                  districtID));
        }
